...
export default async function FollowingPage() {
  ...
  // IDs I'm following (ordered by most recent)
  ...
  const profileMap = new Map(followingProfiles.map((p) => [p.id, p]));
    ...
    <div className="mx-auto max-w-2xl space-y-5">
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-serif text-2xl font-bold text-pine">
          Following <span className="text-base font-normal text-muted">· {sorted.length}件</span>
        </h1>
      </div>

      {sorted.length === 0 ? (
        <Card className="py-12 text-center">
          <p className="text-sm text-muted">まだ誰もフォローしていません。</p>
          <p className="mt-1 text-xs text-muted">Not following anyone yet.</p>
          <Link href="/feed" className="mt-4 inline-block text-sm font-semibold text-moss-600 hover:text-pine">
            Feed で学習者を探す →
          </Link>
        </Card>
      ) : (
