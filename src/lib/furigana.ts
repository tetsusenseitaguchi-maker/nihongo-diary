...
const READING_DICTIONARY_RAW: [string, string][] = [
  ...
  /* ── 長音・末尾モーラの脱落（2026-08 の実測から） ─────────────────────
     「編集」のふりがなが へんしゅく と出た報告を受けて、本番の (語, 読み)
     ペア 145,054件を横断で確認した結果。報告された へんしゅく そのものは
     DB に1件もなく、代わりに同じ語が へんしゅう 39件・へんしゅ 1件 という
     分布で、長音の「う」が落ちる形で壊れていた。
     壊れ方は三つに分かれ、ここで対策できるのは最初の二つだけ：
       1. 長音「う」の脱落  編集→へんしゅ、呪術→じゅじゅ(つ脱落)
       2. 長音「う」→「く」 募集している→ぼしゅくしている（報告と同型・DB内で唯一）
       3. 別漢字との取り違え 帰国→ききょく（国/局。上の国ブロックで対策済み）、
          試飲→ししゅ（飲 に無い音読み）、獣医→じゅういしゃ（医/医者）
     牛→ぎゅ、休→きゅ、少→ちょ、彼→かのじょ も実在するが、1文字語は
     下の length >= 2 フィルタで落ちるのでここには置けない。
     発生率は語によって 2.5%前後で、同じ語が正しく描かれる回数のほうが
     桁で多い。つまりこれは常に壊れる語の一覧ではなく、たまに壊れる語を
     正しい側に固定するための一覧。 */
  ["編集", "へんしゅう"], ["募集", "ぼしゅう"], ["獣医", "じゅうい"], ["呪術", "じゅじゅつ"],
  ["試飲", "しいん"],
  /* 同じ「〜しゅう」族で、日記に出やすく読みが一つに決まるもの。実測では
     まだ壊れていない（予防）。十／自由の「じゅう」は 十 が single-char で
     入れられず、自由 は じゆう で長音ではないので、族としては別扱い。 */
  ["収集", "しゅうしゅう"], ["週間", "しゅうかん"], ["習慣", "しゅうかん"], ["自由", "じゆう"],
